...
export class FanSwitchAccessory {
  ...
  setActive(value) {
	this.platform.log.debug('Triggered SET Active:', Boolean(value));
  ...
  async setTargetState(value, callback: CharacteristicSetCallback) {
    try {
		this.platform.log.debug('Triggered SET Target State:', value);
      const targetState = value as boolean;
      const fanRef = targetState ? this.FAN_ON : this.FAN_AUTO;
	  this.platform.log.debug('Triggered SET Target State:', fanRef);
	  ...
	  this.platform.log.debug('Request Body:', requestBody);
      ...
      this.platform.log.debug(`Set Ecobee to ${fanRef} with result: ${JSON.stringify(response.data)}`);
  ...
  private async checkStatusFromAPI(): Promise<string> {
    try {
      ...
      try {
		...
		this.platform.log.debug('Events:', events);
        if (events.length > 0) {
